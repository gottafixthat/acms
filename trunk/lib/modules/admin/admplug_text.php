<?php
/**
 * admplug_text.php - Admin module for the text handler.
 *
 **************************************************************************
...
if (eregi('admplug_text.php', $_SERVER['PHP_SELF'])) die ("This file may not be loaded directly.");

// The global variables we use to load ourselves into.
global $admplugins;

$admplugins['text'] = new admplugin_text();

// The admplugin_text class.
class admplugin_text {
...
/*
** admplugin_text - Constructor.  This handles the setup of the various admin
**                  functions of this plugin.
*/

function admplugin_text()
{
...
function showEditForm($act = "edit")
{
    ...
    $tpl = new acmsTemplate($this->templatePath . "text_edit_form.tpl");
    ...
    $app->addBlock(10, CONTENT_ZONE, "Edit Text Chunk", $tpl->get());
}
...
function loadChunk($ChunkID)
{
    ...
    $cc = new chunkClass("text");

    if (!$cc->fetchID($ChunkID)) {
        $app->addBlock(0, CONTENT_ZONE, "Admin - Edit Text Chunks", "Unable to load the specified chunk.");
        return false;
    }
...
function processEdit()
{
    ...
    if (!$this->ChunkID) {
        ...
        // Make sure the chunk that we are replacing is a text chunk.
