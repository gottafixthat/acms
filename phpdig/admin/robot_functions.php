...
function phpdigGetSiteFromUrl($id_connect,$url,$linksper) {
    //format url
    $pu = parse_url($url);
    ...
    if (mysql_num_rows($result) > 0) {
        ...
        if ($subpu['ok'] == 1) {
            $query_tempspider = "UPDATE ".PHPDIG_DB_PREFIX."site_page SET num_page = ".$linksper." WHERE site_id = ".$site_id.";";
         ...
         if ($subpu['ok'] == 1) {
            set_time_limit(8600);
            $query = "INSERT INTO ".PHPDIG_DB_PREFIX."site_page SET site_id = ".$site_id.", num_page = ".$linksper.";";
            mysql_query($query,$id_connect);
            if (isset($subpu['path']) && (strlen($subpu['path']) > 0) && LIMIT_TO_DIRECTORY) {
...
if (!$fp) {
  ...
  ."User-Agent: PhpDig/".PHPDIG_VERSION." (+http://www.phpdig.net/robot.php)".END_OF_LINE_MARKER.END_OF_LINE_MARKER;
    ...
    while (!$stop && !feof($fp)) {
          ...
          if (($flag_to_stop_check == 0) && (eregi("^(\r|\n|\r\n|\n\n|\r\r|[[:space:]])+$",$answer))) {
...
if (CONTENT_TEXT == 1) {
    ...
    $file_text_path = TEXT_CONTENT_PATH.$spider_id.'.txt';
...
function phpdigSpiderAddSite($id_connect,$url,$linksper) {
/*
    $added_site = phpdigGetSiteFromUrl($id_connect,$url,$linksper);
